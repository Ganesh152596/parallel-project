...
public class PaymentTest {
	...
	@Test
		public void testCreateAccountForMobile() {
			...
			try {
			service.viewBalance("9999999999");
			} catch (PaymentException e) {
			...
			public void testShowBalanceForName() {
				Payment ra=new Payment();
			ra.setMobileNo("0000000000");
			try {
			service.viewBalance(ra.getMobileNo());
			assertEquals("Ganesh", ra.getName());
			} catch (PaymentException e) {
			assertEquals("The mobile number does not exist",e.getMessage());
			}
			}
			...
			try {
			service.Transfer(ra.getMobileNo(),ra2.getMobileNo(), 230);
			} catch (PaymentException e) {
			assertEquals("Mobile number should contain 10 digits",e.getMessage());
			}
			}
			...
			try {
			assertTrue(service.Transfer(ra.getMobileNo(), ra2.getMobileNo(),  230));
			} catch (PaymentException e) {
			System.out.println(e.getMessage());
			}
			}
			...
			try {
				Payment raa=service.printDetails(ra.getMobileNo());
			assertNotNull(raa);
			} catch (PaymentException e) {
			System.out.println(e.getMessage());
			}
				...
				@Test
					...
					try {
						Payment acc=service.printDetails(ra.getMobileNo());
					assertNotNull(acc);
					} catch (PaymentException e) {
						assertEquals("Mobile number should contain 10 digits",e.getMessage());
					}
				...
				@Test
					...
					try {
						Payment acc=service.printDetails(ac.getMobileNo());
					assertNotNull(acc);
					} catch (PaymentException e) {
						assertEquals("The mobile number does not exist",e.getMessage());
					}
